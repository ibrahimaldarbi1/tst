...
function html(filename, data) {
    ...
    for (const name in data.people) {
		...
		let projectname = data.people[name].projectname ? data.people[name].projectname : '';
		let clade = data.people[name].clade ? data.people[name].clade : '';
                ...
                <li id="${name}">
                    <div class="parents couple">
                        <div class="parents-item ${classname}" data-id="${name}">
                            <div class="parents-item-name">
								${data.people[name].name} ${projectname} ${clade}
							</div>
                            <div class="parents-item-fullname">${fullname}</div>
                            <div class="parents-item-dates">${dateBorn} ${sep} ${dateDied} @ibrahimbinzeyad</div>
                        </div>
                    </div>
                </li>
            </ul>`;
...
const run = async (filename) => {
    let family = await csv2json().fromFile(csvFolder + filename + '.csv', 'UTF-8');
    ...
    for (let i = 0; i < family.length; i++) {
		...
		let projectname = member["Project Name"];
		let clade = member["Clade"];
    ...
    fs.writeFileSync(jsonFolder + filename + '.json', JSON.stringify(data), 'UTF-8');

    let htmlCode = await html(filename, data);
    fs.writeFile(distFolder + filename + '.html', htmlCode, {encoding: 'UTF-8'}, function(err) {
        if (err) {
